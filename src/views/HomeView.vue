    ...
    <div>
      ...
      <div v-else>
        <div class="loading__box" v-if="loading">
          <img class="loading__img" src="../assets/images/loading.png" alt="loading img" />
          Carregando dados...
        </div>
        <table v-else>
          <thead>
          ...
          <tbody>
            <Transactions
              v-on:openModal="showTransactionModal($event)"
...
<script>
import Filter from "../components/FilterTransaction.vue";
  ...
  methods: {
    ...
    updateFilter(newFilter) {
      ...
      this.transactions.forEach((element) => {
        ...
        if (
          element.title.toLowerCase() == this.filterBy.title.toLowerCase() &&
          element.status == this.filterBy.status.toLowerCase()
        ) {
...
.table__tag--processing {
  background-color: rgb(105, 88, 203);
}
.table__tag--processed {
