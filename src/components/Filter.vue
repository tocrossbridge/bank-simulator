<template>
    <div class="search__box">
        <input type="text" placeholder="Pesquise por título" name="title" v-model="filterBy.title" @blur="changeFilter" />

        <select name="status" @change="changeFilter" v-model="filterBy.status">
            <option v-for="status in filters.status" :key="status" :value="status.name">
                {{ status.name }}
            </option>
        </select>

        <p class="alert__text" v-if="alert">Digite título e filtre por status para visualizar os resultados</p>
    </div>
</template>
